...
class FavoritesShimmer(context: Context): ShimmerFrameLayout(context){
    var tvShowsTitle= TextView(context).setText("Favorite TV Shows").color(Color.BLACK).size(20).apply{id=View.generateViewId()}
    var tvShowsRecyclerView= RecyclerView(context).apply{id= View.generateViewId()}
    var moviesTitle= TextView(context).setText("Favorite Movies").color(Color.BLACK).size(20).apply{id=View.generateViewId()}
